...
function getRequestGroupById(id) {
  const seed = db.prepare('SELECT request_number FROM requests WHERE id = ?').get(id);
  if (!seed) return null;
...
router.get('/', authenticateToken, requireProjectAccess(req => req.query.project_id), (req, res) => {
  ...
  res.json(groupRequests(db.prepare(query).all(...params)));
});

// Get single request group
router.get('/:id', authenticateToken, (req, res) => {
  const group = getRequestGroupById(req.params.id);
  if (!group) return res.status(404).json({ error: 'Request not found' });
  if (!hasProjectAccess(req.user.id, req.user.role, group.project_id)) {
...
router.post('/', authenticateToken, requireProjectAccess(req => req.body.project_id), (req, res) => {
  ...
  for (const item of validItems) {
    const product = db.prepare('SELECT id FROM products WHERE id = ? AND project_id = ?').get(item.product_id, project_id);
    if (!product) return res.status(404).json({ error: 'Product not found' });
  }

  const requestNumber = generateRequestNumber(project_id);
  const createdIds = [];
  ...
  const create = db.transaction(() => {
    ...
    for (const item of validItems) {
      const id = uuidv4();
      createdIds.push(id);
      insert.run(id, project_id, requestNumber, item.product_id, req.user.id, req.user.name, location, parseFloat(item.quantity), purpose);
    }

    logAudit(db, req.user.id, 'CREATE', 'requests', requestNumber, null, { ...req.body, request_number: requestNumber, items: validItems }, purpose || 'Request submitted');
  });
...
router.put('/:id', authenticateToken, (req, res) => {
  ...
  for (const item of validItems) {
    const product = db.prepare('SELECT id FROM products WHERE id = ? AND project_id = ?').get(item.product_id, group.project_id);
    if (!product) return res.status(404).json({ error: 'Product not found' });
  }

  const update = db.transaction(() => {
    db.prepare('DELETE FROM requests WHERE request_number = ?').run(group.request_number);
    const insert = db.prepare(`
    ...
    for (const item of validItems) {
      insert.run(uuidv4(), group.project_id, group.request_number, item.product_id, group.requested_by, group.requester_name, location, parseFloat(item.quantity), purpose, group.created_at);
    }

    logAudit(db, req.user.id, 'UPDATE', 'requests', group.request_number, group, { ...req.body, items: validItems }, purpose || 'Request updated');
  });
...
router.delete('/:id', authenticateToken, (req, res) => {
  ...
  db.prepare('DELETE FROM requests WHERE request_number = ?').run(group.request_number);
  logAudit(db, req.user.id, 'DELETE', 'requests', group.request_number, group, null, 'Pending request deleted');
  res.json({ message: 'Request deleted' });
});

// Approve request group
router.patch('/:id/approve', authenticateToken, requireRole('admin', 'store_manager'), (req, res) => {
  const group = getRequestGroupById(req.params.id);
  if (!group) return res.status(404).json({ error: 'Request not found' });
  if (!hasProjectAccess(req.user.id, req.user.role, group.project_id)) {
    return res.status(403).json({ error: 'You do not have access to this project' });
  ...
  const approveAndIssue = db.transaction(() => {
    db.prepare(`UPDATE requests SET status='approved', approved_by=?, approved_at=CURRENT_TIMESTAMP WHERE request_number=?`).run(req.user.name, group.request_number);
    ...
    for (const item of group.items) {
      insertIssue.run(uuidv4(), group.project_id, item.product_id, group.requester_name, null, group.location, group.location, group.request_number, item.quantity, group.purpose, req.user.name, req.user.id);
      recalculateProductStock(db, item.product_id);
    }

    logAudit(db, req.user.id, 'UPDATE', 'requests', group.request_number, group, { status: 'approved' }, 'Request approved and issue created');
  });
...
router.patch('/:id/reject', authenticateToken, requireRole('admin', 'store_manager'), (req, res) => {
  const group = getRequestGroupById(req.params.id);
  if (!group) return res.status(404).json({ error: 'Request not found' });
  if (!hasProjectAccess(req.user.id, req.user.role, group.project_id)) {
    return res.status(403).json({ error: 'You do not have access to this project' });
  ...
  db.prepare(`UPDATE requests SET status='rejected', approved_by=?, approved_at=CURRENT_TIMESTAMP, rejection_reason=? WHERE request_number=?`)
    .run(req.user.name, req.body.reason || 'Rejected by manager', group.request_number);
  logAudit(db, req.user.id, 'UPDATE', 'requests', group.request_number, group, { status: 'rejected', reason: req.body.reason }, req.body.reason || 'Rejected by manager');
