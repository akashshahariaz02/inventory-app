...
function ProductModal({ projectId, product, categories, onSave, onClose }) {
  const [form, setForm] = useState(product || { name: '', category_id: '', size: '', unit: 'Piece', opening_stock: 0, minimum_stock: 0, description: '' });
          ...
          <div className="modal-body">
            ...
            {!product && (
              <div className="form-group">
                ...
                <input className="form-control" type="number" min="0" value={form.opening_stock} onChange={e => set('opening_stock', e.target.value)} />
              </div>
            )}
            <div className="form-group">
                ...
                <tbody>
                  {products.map(p => (
                    <tr key={p.id} className="no-hover">
                      <td><strong>{p.name}</strong>{p.description && <div className="text-muted" style={{fontSize:'11px'}}>{p.description}</div>}</td>
                      <td>{p.category_name ? <span className="badge badge-info">{p.category_name}</span> : '—'}</td>
                      <td>{p.size || '—'}</td>
                      <td>{p.unit}</td>
                      <td className="text-success fw-600">{Number(p.total_in || 0).toLocaleString()}</td>
                      <td className="text-danger fw-600">{Number(p.total_out || 0).toLocaleString()}</td>
                      <td>
